...
namespace {
constexpr std::array<std::uint8_t, 7> license_marker = {
    '2', '7', '7', '4', '3', '1', '6'};
...
std::optional<std::vector<std::uint8_t>> extract_containing_string(
...
std::string legacy_narrator_credential()
{
    ...
    // redistribution. New voice packages may reject it.
...
std::optional<std::string> extract_installed_narrator_credential(
    const std::vector<std::uint8_t>& bytes)
{
    const std::vector<std::uint8_t> utf8_marker(license_marker.begin(),
                                                license_marker.end());
    if (const auto extracted =
            extract_containing_string(bytes, utf8_marker, 1)) {
        const std::string credential(extracted->begin(), extracted->end());
        if (!credential.empty()) {
            return credential;
        }
    }
    ...
    utf16_marker.reserve(license_marker.size() * 2);
    for (const auto byte : license_marker) {
        utf16_marker.push_back(byte);
        utf16_marker.push_back(0);
    }
    if (const auto extracted =
            extract_containing_string(bytes, utf16_marker, 2)) {
        return utf16le_to_utf8(*extracted);
    }
    return std::nullopt;
}
...
std::optional<std::string> installed_narrator_credential()
{
    for (const auto& path : speech_extension_candidates()) {
        ...
        if (auto credential = extract_installed_narrator_credential(*bytes)) {
            return credential;
        }
    }
    return std::nullopt;
