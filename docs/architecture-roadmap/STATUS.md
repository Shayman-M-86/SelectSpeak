...
| H | Bounded admission | G available; coordinate native wakeup/accounting with J | Ready | Define bounded interruptible admission through `PcmPlaybackSession`; implement native capacity wakeups/accounting with J rather than adding polling or a WaveOut adapter. |
