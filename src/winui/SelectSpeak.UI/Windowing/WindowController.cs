...
/// focus, and hides rather than closes. The close button, the tray icon and
/// the backend all toggle that hidden state.
/// </summary>
public sealed class WindowController : IDisposable
{
    ...
    public WindowController(Window window)
    {
        ...
        // Double-clicking the title bar still sends SC_MAXIMIZE even with
        // IsMaximizable off, so the subclass that swallows it is installed here.
        _overlay.InstallSubclass();
    ...
    /// <summary>Show or hide the window - what the close button does.</summary>
    public void Toggle()
    {
        if (IsHidden)
        {
