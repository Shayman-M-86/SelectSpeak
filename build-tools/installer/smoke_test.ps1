...
$verifyDist = Join-Path $buildToolsRoot "tools\verify_dist.py"
...
while ([DateTime]::UtcNow -lt $deadline) {
    if ((Test-Path -LiteralPath $settings -PathType Leaf) -and -not $application.HasExited) {
        Start-Sleep -Seconds 2
        $application.Refresh()
        $started = -not $application.HasExited
        break
    }
